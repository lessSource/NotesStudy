...
- (instancetype)initWithTitle:(NSString *)title handler:(void (^)(zhAlertButton * _Nonnull))handler {
    if (self = [super init]) {
        ...
        _horizontalLine.backgroundColor = [UIColor mainColor].CGColor;
        [self.layer addSublayer:_horizontalLine];
        
        _verticalLine = [CALayer layer];
        _verticalLine.backgroundColor = [UIColor mainColor].CGColor;
...
- (void)layoutSubviews {
    ...
    _horizontalLine.frame = CGRectMake(0, 0, self.frame.size.width, lineWidth);
    _verticalLine.frame = CGRectMake(0, 0, lineWidth, self.frame.size.height);
}
